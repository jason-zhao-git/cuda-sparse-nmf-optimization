...
/*
 * LEVEL 3: COMPUTE-OPTIMIZED GPU IMPLEMENTATION
 *
 * Purpose: Push optimization further to demonstrate diminishing returns
 *
 * Additional Optimizations Beyond Level 2:
 * 1. 8-way ILP (vs 4-way in Level 2) - More latency hiding
 * 2. Block size tuning - Test multiple configurations
 * 3. Register blocking - More computation per thread
 *
 * Expected Performance:
 * - 5-10% improvement over Level 2 (NOT 2x like L1→L2)
 * - Demonstrates hitting Amdahl's Law limits
 * - cuBLAS operations (85% of runtime) can't be improved
 *
 * This level exists to show WHY we can't optimize further,
 * justifying exploration of algorithmic alternatives (sparse).
 */
...
void nmf_compute_opt_gpu(float* h_X, int m, int n, int k, int max_iter, int block_size,
    ...
    printf("========================================\n");
    printf("LEVEL 3: COMPUTE-OPTIMIZED GPU\n");
    printf("========================================\n");
    printf("Matrix: %dx%d, Rank: %d, Iterations: %d\n", m, n, k, max_iter);
    printf("Compute Optimizations:\n");
    printf("  - 8-way ILP (vs 4-way in Level 2)\n");
    printf("  - Block size: %d threads (tunable)\n", block_size);
    printf("  - Maximum latency hiding\n");
    printf("Expected: 5-10%% improvement (hitting ceiling)\n");
    printf("----------------------------------------\n");
    ...
    printf("  Theoretical latency hiding: 2x better than Level 2\n");
    printf("  Practical gain: Limited by memory bandwidth saturation\n");
    printf("----------------------------------------\n\n");
